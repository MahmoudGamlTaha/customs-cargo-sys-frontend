...
const PublicFreeTradeCertificate: React.FC = () => {
  ...
  useEffect(() => {
      ...
      if (
        ...
        (target.closest("[data-language-button]") ||
          ...
          (target.closest("button") &&
            target.closest("button").classList.contains("back-button")) ||
          target.parentElement?.classList.contains("back-button") ||
          target.parentElement?.hasAttribute("data-back-button") ||
          (target.closest("div") &&
            target.closest("div").style.zIndex === "1000") ||
          target.style.zIndex === "1000")
      ) {
        console.log(
          "Button interaction allowed:",
          target.tagName,
          target.className
        );
        return;
    ...
    events.forEach((eventType) => {
      document.addEventListener(eventType, disableAllInteractions, {
        capture: true,
      });
    });
    ...
    return () => {
      events.forEach((eventType) => {
        document.removeEventListener(eventType, disableAllInteractions, {
          capture: true,
        });
      });
    };
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <div>
                <strong>{t("freeTradeCertificate.referenceNumber")}:</strong>{" "}
                {certificateData?.identity_number ||
                  t("freeTradeCertificate.notSpecified")}
              </div>
              <div>
                <strong>{t("freeTradeCertificate.serialNumber")}:</strong>{" "}
                <span
                  style={{
                    direction: "ltr",
              ...
              <div>
                <strong>
                  {t("freeTradeCertificate.certificateTitleField")}:
                </strong>{" "}
          ...
          >
            ...
            >
              <div>
                <strong>{t("freeTradeCertificate.identityNumber")}:</strong>{" "}
                {certificateData?.identity_number ||
                  t("freeTradeCertificate.notSpecified")}
              </div>
              <div>
                <strong>{t("freeTradeCertificate.phoneNumber")}:</strong>{" "}
        ...
        >
          ...
          >
            ...
            >
              <div>
                <strong>{t("freeTradeCertificate.invoiceNumber")}:</strong>{" "}
              ...
              <div>
                ...
                {certificateData?.invoice_date
                  ? new Date(certificateData.invoice_date).toLocaleDateString(
                      "ar-SA"
                    )
                  : t("freeTradeCertificate.notSpecified")}
              </div>
          ...
          >
            ...
            >
              <div>
                <strong>{t("freeTradeCertificate.grossWeight")}:</strong>{" "}
                {certificateData?.weight ||
                  t("freeTradeCertificate.notSpecified")}{" "}
                {t("freeTradeCertificate.kg")}
              </div>
              <div>
                <strong>{t("freeTradeCertificate.netWeight")}:</strong>{" "}
                {certificateData?.net_weight ||
                  t("freeTradeCertificate.notSpecified")}{" "}
                {t("freeTradeCertificate.kg")}
              </div>
              <div>
                <strong>{t("freeTradeCertificate.quantity")}:</strong>{" "}
                {parsedExtraData?.quantity ||
                  certificateData?.quantity ||
                  t("freeTradeCertificate.notSpecified")}
              </div>
              <div>
                <strong>{t("freeTradeCertificate.cost")}:</strong>{" "}
                {parsedExtraData?.item_cost ||
                  t("freeTradeCertificate.notSpecified")}{" "}
                {t("freeTradeCertificate.libyanDinar")}
              </div>
            </div>
          </div>
        </div>
        ...
        >
          ...
          >
            ...
            >
              <div>
                <strong>{t("freeTradeCertificate.countryOfOrigin")}:</strong>{" "}
                {parsedExtraData?.goods_origin ||
                  certificateData?.country_producer ||
                  t("freeTradeCertificate.notSpecified")}
              </div>
              <div>
                <strong>{t("freeTradeCertificate.originStandard")}:</strong>{" "}
                {certificateData?.standard_of_origin ||
                  t("freeTradeCertificate.notSpecified")}
              </div>
              <div>
                <strong>{t("freeTradeCertificate.officialUse")}:</strong>{" "}
                {certificateData?.for_official_use ||
                  t("freeTradeCertificate.notSpecified")}
              </div>
              <div>
                <strong>{t("freeTradeCertificate.originDeclaration")}:</strong>{" "}
                {parsedExtraData?.origin_declaration ||
                  t("freeTradeCertificate.notSpecified")}
              </div>
            </div>
          </div>
              ...
              <div>
                ...
                >
                  {parsedExtraData?.transport_details ||
                    certificateData?.transfer_detail ||
                    t("freeTradeCertificate.notSpecified")}
                </div>
              </div>
              {/* <div>
        ...
        >
          ...
          >
            ...
            >
              <div>
                <strong>{t("freeTradeCertificate.totalValue")}:</strong>{" "}
                {parsedExtraData?.total_value ||
              ...
              <div>
                ...
                >
                  {parsedExtraData?.total_value_text ||
                    t("freeTradeCertificate.notSpecified")}
                </div>
              </div>
              <div>
                <strong>
                  {t("freeTradeCertificate.valueAddedPercentage")}:
                </strong>{" "}
                {parsedExtraData?.value_added_percentage ||
                  t("freeTradeCertificate.notSpecified")}
                {t("freeTradeCertificate.percentage")}
              </div>
              <div>
                <strong>
                  {t("freeTradeCertificate.foreignRevenueQuantity")}:
                </strong>{" "}
                {parsedExtraData?.foreign_revenue_quantity ||
                  t("freeTradeCertificate.notSpecified")}
              </div>
              <div>
                <strong>
                  {t("freeTradeCertificate.foreignRevenueValue")}:
                </strong>{" "}
                {parsedExtraData?.foreign_revenue_value ||
                  t("freeTradeCertificate.notSpecified")}{" "}
                {t("freeTradeCertificate.libyanDinar")}
              </div>
            </div>
          ...
          >
            ...
            >
              <div>
                <strong>{t("freeTradeCertificate.issuePlace")}:</strong>{" "}
                {parsedExtraData?.issue_place ||
                  t("freeTradeCertificate.notSpecified")}
              </div>
              <div>
                <strong>{t("freeTradeCertificate.issueDate")}:</strong>{" "}
                {parsedExtraData?.issue_date
                  ? new Date(parsedExtraData.issue_date).toLocaleDateString(
                      "ar-SA"
                    )
                  : t("freeTradeCertificate.notSpecified")}
              </div>
              <div>
                <strong>
                  {t("freeTradeCertificate.certifyingAuthority")}:
                </strong>{" "}
                {parsedExtraData?.certifying_authority ||
                  t("freeTradeCertificate.notSpecified")}
              </div>
              {/* <div>
